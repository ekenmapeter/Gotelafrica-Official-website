                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-sm text-gray-500 truncate">{{ $data->description }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm font-semibold text-gray-900">
                                    ₦{{ number_format($data->price, 2) }}
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $data->status ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    @if($data->status)
                                    <form method="POST" action="{{ route('product/deactivate', $data->id) }}" class="inline">
                                        @csrf
                                    ...
                                    @else
                                    <form method="POST" action="{{ route('product/activate', $data->id) }}" class="inline">
                                        @csrf
                                    ...
                                    @endif
                                    <button @click="editModalOpen = true; currentProduct = {{ json_encode($data) }}" class="text-blue-600 hover:text-blue-900 text-sm font-medium">
                                        Edit
                                    </button>
                                    <form method="POST" action="{{ route('product/delete', $data->id) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
